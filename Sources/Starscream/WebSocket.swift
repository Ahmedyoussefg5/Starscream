...
open class FoundationStream : NSObject, WSStream, StreamDelegate  {
    ...
    public func connect(url: URL, port: Int, timeout: TimeInterval, ssl: SSLSettings, completion: @escaping ((Error?) -> Void)) {
        ...
        #if os(Linux)
            CFReadStreamSetDispatchQueue(unsafeBitCast(inStream, to: CFReadStream.self),
                                         unsafeBitCast(FoundationStream.sharedWorkQueue, to: OpaquePointer.self))
            CFWriteStreamSetDispatchQueue(unsafeBitCast(outStream, to: CFWriteStream.self),
                                          unsafeBitCast(FoundationStream.sharedWorkQueue,
     ...
     */
    open func stream(_ aStream: Foundation.Stream, handle eventCode: Foundation.Stream.Event) {
...
open class WebSocket : NSObject, StreamDelegate, WebSocketClient, WSStreamDelegate {
    ...
    */
    ...
    private func generateWebSocketKey() -> String {
        ...
        let seed = 16
        for _ in 0..<seed {
            let uni = UnicodeScalar(UInt32(97 + Int.random(in: 0..<25)))
            key += "\(Character(uni!))"
        }
...
private extension String {
    func sha1Base64() -> String {
        return self.data(using: String.Encoding.utf8)!.sha1().base64EncodedString()
    }
}
